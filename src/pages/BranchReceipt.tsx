                    ...
                    <tr key={draft.tempId} className="border-b last:border-0 bg-blue-50 dark:bg-blue-950/30">
                      ...
                      <td className={tdClass}>
                        <div className="flex items-center gap-1">
                          <input type="number" min={0} step="any" defaultValue={draft.qtyReceived || ''} onBlur={e => handleUpdateDraft(draft.tempId, 'qtyReceived', Number(e.target.value))} className="h-8 text-xs text-right w-full font-mono px-1.5 py-1 border rounded-md bg-background" placeholder="0" onFocus={e => e.target.select()} key={`qty-${draft.tempId}-${draft.qtyReceived}`} />
                        </div>
                      </td>
                      ...
                      <td className={tdClass}>
                        <input type="number" min={0} step="any" defaultValue={draft.actualTotalPaid || ''} onBlur={e => handleUpdateDraft(draft.tempId, 'actualTotalPaid', Number(e.target.value))} className="h-8 text-xs text-right w-full font-mono px-1.5 py-1 border rounded-md bg-background" placeholder="0.00" onFocus={e => e.target.select()} key={`paid-${draft.tempId}-${draft.actualTotalPaid}`} />
                      </td>
