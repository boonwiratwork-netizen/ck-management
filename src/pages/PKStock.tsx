...
export default function PKStockPage({ skus, stockData, bomHeaders, bomLines }: Props) {
  ...
  const [pkDailyUsage, setPkDailyUsage] = useState<Record<string, number>>({});
  ...
  type PKRow = {
    ...
    currentStock: number;
    stockValue: number;
    healthStatus: "red" | "green";
  };

  const pkComparators = useMemo(
    () => ({
      skuId: (a: PKRow, b: PKRow) => a.sku.skuId.localeCompare(b.sku.skuId),
      name: (a: PKRow, b: PKRow) => a.sku.name.localeCompare(b.sku.name),
      category: (a: PKRow, b: PKRow) => a.sku.category.localeCompare(b.sku.category),
      currentStock: (a: PKRow, b: PKRow) => a.currentStock - b.currentStock,
      stockValue: (a: PKRow, b: PKRow) => a.stockValue - b.stockValue,
      coverDay: (a: PKRow, b: PKRow) => {
        const aUsage = pkDailyUsage[a.sku.id] || 0;
        const bUsage = pkDailyUsage[b.sku.id] || 0;
  ...
  const filteredRows = useMemo(() => {
    return pkSkus
      ...
      .filter((row) => {
        if (
          search &&
          !row.sku.name.toLowerCase().includes(search.toLowerCase()) &&
          !row.sku.skuId.toLowerCase().includes(search.toLowerCase())
        )
          return false;
  ...
  }, [pkSkus, stockBalances, getStdUnitPrice, getLastReceiptDate, search, filterCategory, filterStorage]);
  ...
  const {
    ...
    sortDir,
    handleSort,
  } = useSortableTable(filteredRows, pkComparators, "skuId", "asc");
  ...
  const coverDayByStorage = useMemo(() => {
    ...
    filteredRows.forEach((row) => {
      const dailyUsage = pkDailyUsage[row.sku.id] || 0;
  ...
  }, [filteredRows, pkDailyUsage]);

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">{t("title.pkStock")}</h2>
          <p className="text-sm text-muted-foreground mt-0.5">Packaging stock balances</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <div className="rounded-lg border bg-card p-4">
          <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("summary.pkSkus")}</p>
          <p className="text-2xl font-bold mt-1">{pkSkus.length}</p>
        </div>
        <div className="rounded-lg border bg-card p-4">
          <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
            {t("summary.totalStockValue")}
          </p>
            ...
            {(["Chilled", "Frozen", "Ambient"] as const).map((s) => (
              <div key={s} className="flex justify-between items-center">
                ...
                <span className="text-sm font-mono font-semibold">
                  {coverDayByStorage[s]}
                  {coverDayByStorage[s] !== "—" && <span className="text-xs text-muted-foreground ml-1">วัน</span>}
                </span>
              </div>
            ))}
          </div>
              ...
              sortedRows.map((row) => {
                ...
                return (
                  <tr key={row.sku.id} className={table.dataRow}>
                    ...
                    <td className={table.truncatedCell} title={row.sku.name}>
                      {row.sku.name}
                    </td>
