...
export default function TransferOrderPage({
  ...
  const {
    pendingTRs, toHistory, historyLoading,
    createTO, updateTOLine, sendTO, cancelTO,
    fetchHistory, fetchTODetail, addTOLine, deleteTOLine,
  ...
  // ─── Create TO from TR ───
  const handleCreateFromTR = useCallback(async (tr: PendingTR) => {
  ...
  // ─── Update line locally ───
  const handleLineUpdate = useCallback((lineId: string, field: 'actualQty' | 'note', value: any) => {
    setFormState(prev => {
      if (!prev) return prev;
      return {
        ...prev,
  ...
  // ─── Add item (standalone) ───
  const handleAddItem = useCallback(async (skuId: string) => {
    if (!formState) return;
  ...
  const sortedHistory = useMemo(() => {
    ...
    list.sort((a, b) => {
      let cmp = 0;
      switch (sortKey) {
        case 'toNumber': cmp = a.toNumber.localeCompare(b.toNumber); break;
  ...
  const hasLinesWithQty = useMemo(
    () => (formState?.lines.some(l => l.actualQty > 0)) ?? false,
    [formState?.lines]
  );
              ...
              <tbody>
                {pendingTRs.map(tr => (
                  <tr key={tr.trId} className={tableTokens.dataRow}>
                    ...
                    <td className={`${tableTokens.dataCell} ${isUrgent(tr.requiredDate) ? 'text-destructive font-medium' : ''}`}>
                      {tr.requiredDate}
                    </td>
        ...
        <div className="rounded-lg border bg-card p-6 space-y-4">
              ...
              <tbody>
                {formState.lines.map((line, idx) => (
                  <tr key={line.id} className={tableTokens.dataRow}>
                    ...
                    <td className={tableTokens.dataCellMono}>
                      {formatNumber(line.actualQty * line.unitCost, 2)}
                    </td>
          ...
          {canEdit && !formState.trId && (
            <div className="flex items-center gap-2">
              ...
              {skuSearchOpen && (
                <Button variant="ghost" size="sm" onClick={() => setSkuSearchOpen(false)}>Cancel</Button>
              )}
            </div>
          )}
            ...
            <tbody>
              {historyLoading ? (
                Array.from({ length: 3 }).map((_, i) => (
                  <tr key={i} className={tableTokens.dataRow}>
                    {Array.from({ length: 8 }).map((_, j) => (
                      <td key={j} className={tableTokens.dataCell}><Skeleton className="h-4 w-full" /></td>
                    ))}
                  </tr>
                ))
                ...
                <tr key={to.id} className={tableTokens.dataRow}>
                  ...
                  <td className={tableTokens.truncatedCell} title={to.branchName}>{to.branchName}</td>
                  ...
                  <td className={tableTokens.dataCell}>
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${toStatusBadge[to.status] || ''}`}>
                      {to.status}
                    </span>
                  </td>
                  <td className={`${tableTokens.dataCell} text-center`}>
                    <div className="flex items-center justify-center gap-1">
                      ...
                      {isManagement && to.status !== 'Cancelled' && to.status !== 'Received' && (
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={() => cancelTO(to.id)} title="Cancel">
        ...
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto print:max-w-full print:max-h-full print:overflow-visible">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-3">
              ...
              {detailTO && (
                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${toStatusBadge[detailTO.status] || ''}`}>
                  {detailTO.status}
                </span>
              )}
            </DialogTitle>
                    ...
                    <tbody>
                      {detailLines.map(l => (
                        <tr key={l.id} className={tableTokens.dataRow}>
                          ...
                          <td className={`${tableTokens.dataCellMono} text-muted-foreground`}>{formatNumber(l.plannedQty, 0)}</td>
                          ...
                          <td className={tableTokens.dataCellMono}>฿{formatNumber(l.lineValue, 2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
