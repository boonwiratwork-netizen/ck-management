...
export function useTransferOrder(getBomCostPerGram?: (skuId: string) => number) {
  ...
  const sendTO = useCallback(
    async (toId: string, lines: TOLine[]): Promise<{ error?: string }> => {
      ...
      const { data: to } = await supabase.from("transfer_orders").select("tr_id, to_number").eq("id", toId).single();
  ...
  const saveTOEdits = useCallback(async (toId: string, lines: TOLine[]) => {
    ...
    // Update each line with new actual_qty + line_value
    let totalValue = 0;
    ...
    for (const tl of dbLines || []) {
      if (tl.actual_qty > 0) {
        ...
        if (insErr) console.error("Re-insert adjustment failed:", insErr.message);
      }
    }
  ...
  const fetchHistory = useCallback(
    async (filters?: { branchId?: string; status?: string; dateFrom?: string; dateTo?: string }) => {
      ...
      let query = supabase
        .from("transfer_orders")
        .select("id, to_number, delivery_date, branch_id, tr_id, total_value, status, notes, created_at, updated_at")
      ...
      if (error) {
        toast.error("Failed to load TO history");
