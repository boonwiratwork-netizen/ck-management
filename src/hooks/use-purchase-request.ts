...
export function usePurchaseRequest(branchId: string | null) {
  ...
  const fetchHistory = useCallback(async (filters?: {
    ...
    if (error) { toast.error('Failed to load PR history'); setHistoryLoading(false); return; }
  ...
  const fetchPRDetail = useCallback(async (prId: string): Promise<PRDetailLine[]> => {
    const { data, error } = await supabase
      .from('purchase_request_lines')
      .select('id, sku_id, requested_qty, uom, suggested_qty, stock_on_hand, avg_daily_usage, rop, pack_size, supplier_id')
      .eq('pr_id', prId);
  ...
  const cancelPR = useCallback(async (prId: string) => {
  ...
  const getPendingPRCountsBySupplier = useCallback(async (targetBranchId?: string): Promise<Record<string, number>> => {
