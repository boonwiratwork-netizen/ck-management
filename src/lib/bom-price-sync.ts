...
export async function computeBomCostFromDb(headerId: string): Promise<{ costPerGram: number; smSkuId: string }> {
  const { data: header } = await supabase.from("bom_headers").select("*").eq("id", headerId).single();
  ...
  if (header.bom_mode === "simple") {
    const { data: dbLines } = await supabase.from("bom_lines").select("*").eq("bom_header_id", headerId);
    mainOutput = header.batch_size * header.yield_percent;
    totalCost = (dbLines || []).reduce((s, l) => {
      const lineYield = l.yield_percent ?? 1.0;
      const effQty = lineYield > 0 ? l.qty_per_batch / lineYield : l.qty_per_batch;
      return s + effQty * getCost(l.rm_sku_id);
    }, 0);
  } else {
    // Multi-step
...
export async function syncBomPrice(skuId: string, costPerGram: number): Promise<number> {
  ...
  if (existing) {
    ...
    await supabase.from("prices").update({ is_active: false }).eq("sku_id", skuId).eq("supplier_id", BOM_SUPPLIER_ID);
 ...
 */
export async function cascadeBomCost(
  smSkuId: string,
  newCostPerGram: number,
): Promise<{ menuBomCount: number; spBomCount: number }> {
  let menuBomCount = 0;
  let spBomCount = 0;
  ...
  if (menuLines && menuLines.length > 0) {
    for (const line of menuLines) {
      const effQty =
        line.effective_qty ??
        (line.yield_pct > 0 ? line.qty_per_serving / (line.yield_pct / 100) : line.qty_per_serving);
    ...
    for (const spSkuId of spSkuIds) {
      ...
      if (allSpLines && allSpLines.length > 0) {
        ...
        for (const l of allSpLines) {
          const lineCost =
            batchYield > 0 ? (l.qty_per_batch * (priceMap.get(l.ingredient_sku_id) ?? 0)) / batchYield : 0;
